...
class OcrKing {
    ...
    public function __construct($apikey) {
        if (isset($apikey) && !empty($apikey)) {
            ...
            $this->status = false;
            $this->errormsg = 'apiKey can not be null ';
            return;
        }
    }
    ...
    public function doOcrKing($setting = array()) {
        ...
        if (!empty($this->ocrfile)) {
            ...
            if (empty($this->postdata ['type'])) {
                $this->status = false;
                $this->errormsg = 'type must be set with the original url of captcha image';
                return;
            } else {
    ...
    public function setFilPath($file) {
        if (file_exists($file)) {
            if (class_exists('\CURLFile')) {
                $this->ocrfile = new \CURLFile($file);
            } else {
                $this->ocrfile = '@' . $file;
            }
        }
